...
public interface TableCategoriesMapNames extends DatabaseNames, TableCategories, TableRecipesNames {
	...
	String createTableCategoriesMapCommand = "CREATE TABLE `" + databaseName + "`.`" + tableCategoriesMapName + "` ("
			...
			+ " BIGINT UNSIGNED, FOREIGN KEY (" + tableCategoriesMapColumnRecipeId + ") REFERENCES " + tableRecipesName
			+ "(" + tableRecipesColumnID + "), FOREIGN KEY (" + tableCategoriesMapColumnCategoryId + ") REFERENCES "
	...
	String queryGetRecipesNumberByCategory = "SELECT COUNT(*) AS count FROM `" + databaseName + "`.`"
			+ tableCategoriesMapName + "` WHERE " + tableCategoriesMapColumnCategoryId + "='";
}
